...
/**
 * Invoice Serializer — Persistence Shape -> API Shape
 *
 * ARCHITECT'S NOTE:
 *
 * This is the boundary that converts Decimal columns into precision-safe
 * strings (see invoice-response.type.ts for the rationale). It is defined
 * against EXPLICIT structural input interfaces (InvoiceEntity / LineItemEntity)
 * rather than the raw Prisma types, which keeps the serializer decoupled and
 * unit-testable in isolation. The Prisma row satisfies these interfaces
 * structurally, so the service can pass its query result straight through.
 *
 * Scales mirror the schema: money = 4dp, quantity = 2dp, rates = 4dp.
 */
