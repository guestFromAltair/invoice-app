...
- **Multi-currency Support:** Save the exact exchange rate the moment an invoice is created so currency changes 
    don't mess up the accounting later.
- **Recurring Invoicing:** Create an automated background scheduler to auto-generate and send regular monthly invoices 
    for subscription clients.
- **Email Reminders:** Hook up an email service to automatically send gentle payment reminders when an invoice goes past 
    its due date.

### System Upgrades & Performance
- **Idempotency Key Retention & Archiving:** Create an automated background cleanup task to purge or archive old, 
    expired idempotency keys from the PostgreSQL database. This keeps the active transaction tables small and ensures 
    lookup queries stay lightning-fast as the business scales.
- **External API Integrations:** Build out a robust retry mechanism for sending invoice updates to external business 
    partners, ensuring our main application doesn't slow down if their servers are temporarily offline.

---
---
*Built as a portfolio project targeting fintech and banking Java engineering roles.*
*Designed to demonstrate core financial system patterns, such as database-backed idempotency, append-only audit tracking,*
*and automated ledger reconciliation using enterprise Spring Boot practices.*